...
class ParcelCreateValidationTest {
    ...
    @Test
    fun `negative weight causes validation error`() = runBlocking {
        val ctx = validContext().copy(pdRequest = validContext().pdRequest.copy(weight = Double.MIN_VALUE))
        processor.exec(ctx)
        assertTrue(ctx.errors.any { it.code == "validation-weight-not-positive" })
        assertEquals(PDState.FAILING, ctx.state)
    }
